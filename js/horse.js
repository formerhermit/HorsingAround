...
/**
 * Wardrobe layers, split by where they sit so the head-worn pieces can ride
 * inside the head group (which is nudged toward the body) while leg/back
 * pieces stay put. `wardrobe` is the horse's owned shop item ids (shop.js).
 */
function bodyAccessoryMarkup(wardrobe = []) {
  let markup = '';
...
function headAccessoryMarkup(wardrobe = []) {
  let markup = '';
  if (wardrobe.includes('scarf')) {
    // shifted back toward the neck's back edge and tilted clockwise so its
    // back edge lifts up onto the neck while the front dips a little
    markup += `<g transform="translate(-5,1) rotate(10 128,76)"><path d="M113,60 Q129,71 143,57 Q146,68 141,77 Q124,87 109,71 Q107,65 113,60 Z" fill="#D9534F"/><path d="M115,74 Q108,90 115,103 Q124,92 119,76 Z" fill="#C0392B"/></g>`;
  }
...
export function horseSVG(horse, wardrobe = []) {
  ...
  // Base artwork faces right; mirror the whole group around the viewBox's
  // horizontal span (-2 to 190) for horses that arrived facing left.
  const flip = horse.facing === 'left';
  return `
<svg viewBox="-2 -4 192 170" role="img" aria-label="${horse.name} the horse" xmlns="http://www.w3.org/2000/svg">
  <g${flip ? ' transform="translate(188,0) scale(-1,1)"' : ''}>
  <!-- ground shadow -->
  ...
  <!-- hooves: colour band over each leg's foot, matching the rounded bottom
       (rx=5) exactly so the leg silhouette is unchanged, only recoloured -->
  <path d="M50,143 L61,143 L61,147 A5,5 0 0 1 56,152 L55,152 A5,5 0 0 1 50,147 Z" fill="#3B2E20"/>
  <path d="M68,144 L79,144 L79,148 A5,5 0 0 1 74,153 L73,153 A5,5 0 0 1 68,148 Z" fill="#3B2E20"/>
  <path d="M98,144 L109,144 L109,148 A5,5 0 0 1 104,153 L103,153 A5,5 0 0 1 98,148 Z" fill="#3B2E20"/>
  <path d="M116,143 L127,143 L127,147 A5,5 0 0 1 122,152 L121,152 A5,5 0 0 1 116,147 Z" fill="#3B2E20"/>
  <!-- body -->
  <ellipse data-part="coat" cx="85" cy="92" rx="52" ry="30" fill="${c.coat}"/>
  <!-- neck: base stays anchored to the body; the top is shifted down/in to
       match the head group below, so the neck reads a little shorter -->
  <path data-part="coat" d="M106,82 Q114,51 122,38 L147,47 L131,94 Z" fill="${c.coat}"/>
  <!-- leg/back wardrobe: under the head group so the blanket sits behind the scarf -->
  <g class="accessories">${bodyAccessoryMarkup(wardrobe)}</g>
  <!-- head, mane, face and head-worn wardrobe move together, pulled a touch
       toward the body to shorten the neck; the lowest mane strand then falls
       over the body -->
  ...
  <path data-part="mane" d="M128,16 q-7,9 1,18" fill="none" stroke="${c.mane}" stroke-width="5" stroke-linecap="round"/>
  <path data-part="mane" d="M122,28 q-7,9 1,18" fill="none" stroke="${c.mane}" stroke-width="5" stroke-linecap="round"/>
  <path data-part="mane" d="M118,40 q-6,8 1,16" fill="none" stroke="${c.mane}" stroke-width="5" stroke-linecap="round"/>
  <path data-part="mane" d="M115,52 q-6,7 1,14" fill="none" stroke="${c.mane}" stroke-width="5" stroke-linecap="round"/>
  ...
  <g class="accessories">${headAccessoryMarkup(wardrobe)}</g>
  </g>
  </g>
</svg>`;
}
