...
// already have a save of its own does the game offer signInWithGoogle() as
// an explicit, separate follow-up ("load that save here instead"), because
// that action *does* replace the local game with the other account's.
//
...
//
//  - signInWithGoogle(): sign in fresh. Resolves to whichever account this
//    Google identity belongs to -- an existing one if already linked
//    elsewhere (replacing the local session, the actual "load my save here"
//    case), or a brand-new empty one if it's never been linked at all.
//
...
// the redirect URL (?google=linked|signin) tells main.js which flow just
// finished when the page reloads on return, so it can react accordingly --
// simpler and more robust than trying to time an onAuthStateChange listener
// against Supabase's own URL-token parsing on boot.
...
export async function signInWithGoogle() {
  if (!isConfigured()) return;
  const client = await getClient();
  await client.auth.signInWithOAuth({ provider: 'google', options: { redirectTo: redirectTarget('signin') } });
}
